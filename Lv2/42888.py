...
def solution(record):
    ...
    for mes in record:      # record를 하나씩 받아옴
        if mes.split(" ")[0] == "Enter":                                            # 빈칸을 기준으로 split을 하고 첫번째가 Enter 이라면
            answer.append("{}님이 들어왔습니다.".format(userdict[mes.split(" ")[1]]))   # answer에 userdict에 mes.split(" ")[1] 값 남이 들어왔습니다. 를 추가
        
        elif mes.split(" ")[0] == "Leave":                                          # 빈칸을 기준으로 split을 하고 첫번째가 Leave 이라면
            answer.append("{}님이 나갔습니다.".format(userdict[mes.split(" ")[1]]))     # answer에 userdict에 mes.split(" ")[1] 값 남이 나갔습니다. 를 추가
